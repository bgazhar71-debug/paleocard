      ...
      <div class="modal-body">
        <form action="{{ url('users/'.$item->id) }}" method="post">
            ...
            <div class="mb-3">
                ...
                <input type="email" class="form-control @error('email') is-invalid @enderror" id="email" name="email" value="{{ old('email', $item->email) }}">
                @error('email')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div class="mb-3">
                ...
                <input type="password" class="form-control @error('konfirmasi_password') is-invalid @enderror" id="konfirmasi_password" name="password_confirmation" value="{{ old('konfirmasi_password') }}">
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="role" name="role" required>
                    <option value="0" {{ old('role', $item->role) == 0 ? 'selected' : '' }}>Member</option>
                    <option value="1" {{ old('role', $item->role) == 1 ? 'selected' : '' }}>Admin</option>
                </select>
            </div>
