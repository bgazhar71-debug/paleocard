...
<div class="container mt-4 mb-5">
    ...
    @endif
    <form action="{{ route('my-articles.update', $article) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="row">
            ...
            <div class="col-md-6 mb-3">
                ...
                <select class="form-select" id="category_id" name="category_id" required>
                    ...
                    @foreach ($categories as $item)
                        <option value="{{ $item->id }}" {{ old('category_id', $article->category_id) == $item->id ? 'selected' : '' }}>
                            {{ $item->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" id="myeditor" name="desc" rows="8" required>{{ old('desc', $article->desc) }}</textarea>
        </div>
            <div class="mb-3">
                ...
                <div class="mt-2">
                    @if($article->img)
                        <img src="{{ asset('storage/back/' . $article->img) }}" class="img-thumbnail img-preview" width="120">
                    @else
        ...
        <div class="row">
            ...
            <div class="col-md-6 mb-3">
                ...
                <input type="date" class="form-control" id="publish_date" name="publish_date" value="{{ old('publish_date', $article->publish_date && !is_string($article->publish_date) ? $article->publish_date->format('Y-m-d') : $article->publish_date) }}">
            </div>
        </div>
        <div class="float-end">
