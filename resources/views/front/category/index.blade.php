...
@section('title', 'Category ' . $category->name . ' - PaleoAtlas')

@section('content')
    <div class="container py-4">
        <div class="mb-4" data-aos="fade-down">
            <form action="{{ route('search') }}" method="post" role="search" aria-label="Search articles">
                @csrf
                <div class="input-group">
                    ...
                    <a href="{{ url('category/'.$category->slug) }}" class="btn btn-outline-secondary" title="Reset search">Reset</a>
                    <button class="btn btn-primary rounded-end-pill" id="button-search" type="submit" aria-label="Submit search">Search</button>
                </div>
            </form>
        </div>
        <div class="alert alert-info py-2">
            Showing articles with category: <b>{{ $category->name }}</b>
        </div>
        <div class="row">
                    ...
                    <div class="card h-100 shadow-sm border-0">
                        ...
                        <div class="card-body d-flex flex-column">
                            <div class="small text-muted mb-2">
                                ...
                                <span class="mx-1">|</span>
                                @if(isset($item->user))
                                    <span class="ms-2"><i class="bi bi-person"></i> {{ $item->user->name }}</span>
                                @endif
                                @if(isset($item->comments_count))
                                    <span class="badge bg-secondary ms-2"><i class="bi bi-chat"></i> {{ $item->comments_count }}</span>
                                @endif
                            </div>
